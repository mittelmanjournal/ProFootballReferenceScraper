...
public class Game {
	...
	private boolean arePosAcronymsValid(Document doc) {
		...
		String[] curPosAcronyms = allStarters.select("td").text().split(" ");
		if (curPosAcronyms.length != 44)
			return false;
		String[] validAcronyms = "QB Z X RB HB TB FB LH RH BB B WB WR RWR LWR FL TE SE RE LE DL LDE DE LDT DT NT UT MG DG RDT RDE LOLB RUSH OLB LLB LILB WILL ILB SLB MLB MIKE WLB RILB RLB ROLB SAM LB LCB CB RCB SS FS LDH RDH S RS DB OL LT LOT T LG G C RG RT ROT"
				.split(" ");
	...
	public Game(String link) {
		...
		if (areStartersValid(gamePage) && arePosAcronymsValid(gamePage)) { //TODO the only things that need to be valid are parameters to the NN, meaning OU doesn't have to be valid
			...
			visDID = getPosIDsFromDoc(gamePage,"vis", "RE LE DL LDE DE LDT DT NT UT MG DG RDT RDE LOLB RUSH OLB LLB LILB WILL ILB SLB MLB MIKE WLB RILB RLB ROLB SAM LB LCB CB RCB SS FS LDH RDH S RS DB".split(" "), visDID.length);
			...
			homeDID = getPosIDsFromDoc(gamePage, "home", "RE LE DL LDE DE LDT DT NT UT MG DG RDT RDE LOLB RUSH OLB LLB LILB WILL ILB SLB MLB MIKE WLB RILB RLB ROLB SAM LB LCB CB RCB SS FS LDH RDH S RS DB".split(" "), homeDID.length);
		}
